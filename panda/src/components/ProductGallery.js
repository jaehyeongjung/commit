...
function ProductGallery() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const visibleProducts = remainingProducts.slice(
          0,
          getVisibleAllProductsCount()
        ); // 화면 크기에 맞게 표시할 상품 수 결정
  ...
  const handleProductClick = (productId) => {
    navigate(`/items/${productId}`); // 상세 페이지로 이동
  };
          ...
          <div className="productGalleryBestMain">
            ...
            {bestProducts.length === 0 ? (
              ...
              bestProducts.map((product, index) => (
                <div
                  className="productGalleryBestMainBox"
                  key={index}
                  onClick={() => handleProductClick(product.id)} // 클릭 시 상세 페이지로 이동
                >
          ...
          <div className="productGalleryAllMain">
            {displayProducts.length === 0 ? (
              ...
              displayProducts.map((product, index) => (
                <div
                  className="productGalleryAllMainBoxItem"
                  key={index}
                  onClick={() => handleProductClick(product.id)} // 클릭 시 상세 페이지로 이동
                >
